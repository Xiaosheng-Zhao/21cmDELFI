...
class Delfi():
    ...
    def __init__(self, data, prior, nde, \
        ...
        #self.sess = tf.Session(config = tf.ConfigProto(intra_op_parallelism_threads=1, inter_op_parallelism_threads=1))
    ...
    def run_simulation_batch(self, n_batch, ps, simulator, compressor, simulator_args, compressor_args, seed_generator = None, sub_batch = 1,n_noise=10):
        ...
        # Dimension outputs
        data_samples = np.zeros((n_batch*sub_batch*n_noise, self.D))
        parameter_samples = np.zeros((n_batch*sub_batch*n_noise, self.npar))
        ...
        while i_acpt <= self.inds_acpt[-1]:
            try:
                sims = simulator(ps[i_prop,:], seed_generator(), simulator_args, sub_batch,n_noise=n_noise)
                
                # Make sure the sims are the right shape
                if sub_batch == 1 and len(sims) != 1:
                    sims = np.array(sims)
                compressed_sims = np.array([compressor(sims[k], compressor_args) for k in range(sub_batch*n_noise)])
                if np.all(np.isfinite(compressed_sims.flatten())):
                    data_samples[i_acpt*sub_batch*n_noise:i_acpt*sub_batch*n_noise+sub_batch*n_noise,:] = compressed_sims
                    parameter_samples[i_acpt*sub_batch*n_noise:i_acpt*sub_batch*n_noise+sub_batch*n_noise,:] = np.tile(ps[i_prop,:], (n_noise, 1))
                    i_acpt += 1
                    if self.progress_bar:
                        pbar.update(1)
                else:
    ...
    # EMCEE sampler
    def emcee_sample(self, log_target=None, x0=None, burn_in_chain=10, main_chain=1000):
        ...
        #return chain, weights, log_prob, weights2
    ...
    def emcee_sample_single(self, log_target=None, x0=None, burn_in_chain=100, main_chain=1000,n=2):
        ...
        #for i in range(1):
            # default log target
            if log_target is None:
                log_target = lambda x: self.log_posterior_individual(i,x, self.data)
    ...
    def emcee_sample_multi(self, log_target=None, x0=None, burn_in_chain=100, main_chain=1000,n=2,data_more=None):
        ...
        for i in range(len(data_more)):
            ...
            if log_target is None:
                #log_target = lambda x: self.log_posterior_stacked(x, self.data[i])
                log_target = lambda x: self.log_posterior_stacked(x, data_more[i])
            ...
            # pull out the log probabilities
            log_prob, weights2 = np.unique(sampler.get_log_prob(flat=True), axis=0, return_counts=True)
            np.save('/scratch/zxs/delfi_fast/emcee_sample/chain_%d.npy'%(i+n),(chain,weights,log_prob,weights2))
            #np.save('/scratch/zxs/delfi_fast/emcee_sample/weights_%d.npy'%(i+n),weights)
            #np.save('/scratch/zxs/delfi_fast/emcee_sample/log_prob_%d.npy'%(i+n),log_prob)
            #np.save('/scratch/zxs/delfi_fast/emcee_sample/weights2_%d.npy'%(i+n),weights2)

        return 0

    def emcee_sample_multi2(self, log_target=None, x0=None, burn_in_chain=100, main_chain=1000,n=2,data_more=None):
        ...
        for i in range(len(data_more)):
            ...
            if log_target is None:
                #log_target = lambda x: self.log_posterior_stacked(x, self.data[i])
                #log_target = lambda x: self.log_posterior_stacked(x, data_more[i])
                log_target = lambda x: self.log_posterior_individual(0,x, data_more[i])
    ...
    def emcee_sample_single_conv(self, log_target=None, x0=None, burn_in_chain=100, main_chain=1000,n=2,data_more=None):
        ...
        if log_target is None:
            #log_target = lambda x: self.log_posterior_stacked(x, self.data[i])
            #log_target = lambda x: self.log_posterior_stacked(x, data_more[i])
            log_target = lambda x: self.log_posterior_individual(0,x, data_more)
        ...
        # pull out the log probabilities
        log_prob, weights2 = np.unique(sampler.get_log_prob(flat=True), axis=0, return_counts=True)
        np.save('/scratch/zxs/delfi_fast/emcee_sample/conv_sample/po_%d.npy'%(n),(chain,weights,log_prob,weights2))
        print (n)

        return 0

    def emcee_sample_single_conv2(self, log_target=None, x0=None, burn_in_chain=100, main_chain=1000,data_more=None):
        pool = multiprocessing.Pool(processes=20)
        num = range(800)
        pool.map(self.emcee_samplei, num)
        #self.emcee_samplei(i,log_target=None, x0=None, burn_in_chain=burn_in_chain, main_chain=main_chain,n=n,data_more=data_more)

    def emcee_samplei(self, i,log_target=None, x0=None, burn_in_chain=2000, main_chain=3000,data_more=None):
        data_more=np.loadtxt('/scratch/zxs/scripts/3DCNN/predmore.txt')[0:800]
        # default log target
        if log_target is None:
            #log_target = lambda x: self.log_posterior_stacked(x, self.data)
            log_target = lambda x: self.log_posterior_individual(i,x, data_more[i])
        ...
        #log_prob, _ = np.unique(sampler.get_log_prob(flat=True), axis=0, return_counts=True)
        log_prob, weights2 = np.unique(sampler.get_log_prob(flat=True), axis=0, return_counts=True)
        np.save('/scratch/zxs/delfi_fast/emcee_sample/conv_sample/po_%d.npy'%(i),(chain,weights,log_prob,weights2))
    ...
    def sequential_training(self, simulator, compressor, n_initial, n_batch, n_populations, proposal = None, \
                            ...
                            validation_split = 0.1, epochs = 300, patience = 20, seed_generator = None, \
                            save_intermediate_posteriors = True, sub_batch = 1,n_noise=10):
        ...
        xs_batch, ps_batch = self.run_simulation_batch(n_initial, ps, simulator, compressor, simulator_args, compressor_args, seed_generator = seed_generator, sub_batch = sub_batch,n_noise=n_noise)

        # Train on master only
        if self.rank == 0:

            # Construct the initial training-set
            self.load_simulations(xs_batch, ps_batch,n_noise)
            ...
            if save_intermediate_posteriors:
                ...
                x0 = self.posterior_samples[np.random.choice(np.arange(len(self.posterior_samples)), p=self.posterior_weights.astype(np.float32)/sum(self.posterior_weights), replace=False, size=self.nwalkers),:]
                self.posterior_samples, self.posterior_weights, self.log_posterior_values, _,_= self.emcee_sample(x0=x0, main_chain=self.posterior_chain_length)
        ...
        for i in range(n_populations):
            ...
            if self.rank == 0:
                ...
                self.proposal_samples, self.proposal_weights, self.log_proposal_values,_,_ = \
            ...
            if self.rank == 0:
                ...
                # Augment the training data
                self.add_simulations(xs_batch, ps_batch,n_noise)
                ...
                if save_intermediate_posteriors:
                    print('Sampling approximate posterior...')
                    x0 = self.posterior_samples[np.random.choice(np.arange(len(self.posterior_samples)), p=self.posterior_weights.astype(np.float32)/sum(self.posterior_weights), replace=False, size=self.nwalkers),:]
                    self.posterior_samples, self.posterior_weights, self.log_posterior_values,_,_ = \
    ...
    def train_ndes(self, training_data=None, batch_size=50, validation_split=0.1, epochs=2000, patience=20, mode='samples'):
    ...
    def load_simulations(self, xs_batch, ps_batch,n_noise):
        ...
        self.n_sims += len(ps_batch)//n_noise
    
    def add_simulations(self, xs_batch, ps_batch,n_noise):
