...
class Delfi():
    ...
    def __init__(self, data, prior, nde, \
        ...
        #self.sess = tf.Session(config = tf.ConfigProto(intra_op_parallelism_threads=1, inter_op_parallelism_threads=1, allow_soft_placement=True, device_count = {'CPU': 1}))
    ...
    def log_likelihood_individual(self, i, theta, data):
        ...
        lnL = self.nde[i].eval((np.atleast_2d((theta-self.p_mean)/self.p_std), np.atleast_2d((data-self.x_mean)/self.x_std)), self.sess).astype(np.float64)
    ...
    def log_likelihood_stacked(self, theta, data):
        ...
        for n in range(self.n_ndes):
            ...
            L += self.stacking_weights[n]*np.exp((self.nde[n].eval((np.atleast_2d((theta-self.p_mean)/self.p_std), np.atleast_2d((data-self.x_mean)/self.x_std)), self.sess)).astype(np.float64))
        #lnL = np.log(L).astype(np.float32)
        lnL = np.log(L)
    ...
    def run_simulation_batch(self, n_batch, ps, simulator, compressor, simulator_args, compressor_args, seed_generator = None, sub_batch = 1):
        ...
        # Dimension outputs
        data_samples = np.zeros((n_batch*sub_batch*self.n_noise, self.D))
        parameter_samples = np.zeros((n_batch*sub_batch*self.n_noise, self.npar))
        ...
        while i_acpt <= self.inds_acpt[-1]:
            try:
                ...
                if np.all(np.isfinite(compressed_sims.flatten())):
                    data_samples[i_acpt*sub_batch*self.n_noise:i_acpt*sub_batch*self.n_noise+sub_batch*self.n_noise,:] = compressed_sims[0]
                    parameter_samples[i_acpt*sub_batch*self.n_noise:i_acpt*sub_batch*self.n_noise+sub_batch*self.n_noise,:] = np.tile(ps[i_prop,:], (self.n_noise, 1))
                    i_acpt += 1
                    if self.progress_bar:
                        pbar.update(1)
                else:
    ...
    def load_simulations(self, xs_batch, ps_batch):
        ...
        self.n_sims += len(ps_batch)//self.n_noise
