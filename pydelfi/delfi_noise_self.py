        ...
        while i_acpt <= self.inds_acpt[-1]:
            try:
                ...
                if np.all(np.isfinite(compressed_sims.flatten())):
                    data_samples[i_acpt*sub_batch*self.n_noise:i_acpt*sub_batch*self.n_noise+sub_batch*self.n_noise,:] = compressed_sims
                    parameter_samples[i_acpt*sub_batch*self.n_noise:i_acpt*sub_batch*self.n_noise+sub_batch*self.n_noise,:] = np.tile(ps[i_prop,:], (n_noise, 1))
                    i_acpt += 1
                    if self.progress_bar:
                        pbar.update(1)
                else:
