...
class Delfi():
    ...
    def multi_noise(self,pp,z0):
        kSpline = np.array([0.15, 0.27142857, 0.39285714, 0.51428571, 0.63571429, 0.75714286, 0.87857143, 1.])
        error2=np.loadtxt('/scratch/zxs/delfi_fast/err/thermal/TotalError_HERA127_PS_100Mpc_z%.6f_FaintGalaxies_1000hr.txt'%z0)
    ...
    def run_simulation_batch(self, n_batch, ps, simulator, compressor, simulator_args, compressor_args, seed_generator = None, sub_batch = 1):
        ...
        #print (self.inds_acpt[-1],flush=True)
        ...
        while i_acpt <= self.inds_acpt[-1]:
            try:
                ...
                #print (i_acpt,flush=True)
                #print (ps[i_prop,:],flush=True)
        ...
        '''
        theta=parameter_samples
        noised_tot=data_samples
        print (theta,flush=True)
        print (theta.shape,flush=True)
        print (noised_tot,flush=True)
        print (noised_tot.shape,flush=True)
        return noised_tot, theta
        ''' 
        data_samples = self.complete_array(data_samples)
        parameter_samples = self.complete_array(parameter_samples)
        
        n_noise=10
        theta=np.repeat(parameter_samples, n_noise, axis=0)
        redshift=[7.668188,8.003182,8.357909,8.716638,9.108561,9.524624,9.966855,10.437500,10.939049,11.474274]
        data_samples=np.repeat(data_samples, n_noise, axis=0)
        noised_tot=np.zeros((data_samples.shape[0],data_samples.shape[1]))
        for nx in range(data_samples.shape[0]):
            for red in range(10):
                noised_tot[nx,8*red:8*(red+1)]=self.multi_noise(data_samples[nx,8*red:8*(red+1)],redshift[red])
    ...
    def emcee_sample(self, log_target=None, x0=None, burn_in_chain=100, main_chain=1000):
        ...
        return chain, weights, sampler.get_chain(flat=True) 
    ...
    def sequential_training(self, simulator, compressor, n_initial, n_batch, n_populations, proposal = None, \
        ...
        #print (ps,flush=True)
        #print (self.inds_acpt,flush=True)
        ...
        if self.rank == 0:
            ...
            if save_intermediate_posteriors:
                ...
                np.savetxt(f, self.log_posterior_values)
                f.close()
